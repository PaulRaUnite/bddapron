...
val vectorsupport_cond : 'a expr array -> 'a Cudd.Bdd.t
...
val make : (('a,'b,'c,'d) #Env.O.t as 'e) -> 'b Expr0.t -> ('e,'b) expr
  (** Creation from an expression without environment *)
...
val vectorsupport_cond : ('a,'b) t array -> 'b Cudd.Bdd.t
    (** Return the support of an array of expressions as a conjunction of the
      BDD identifiers involved in the expressions *)
...
val check_envvar : ('a,'b,'c,'d) #Env.O.t -> string -> unit
val check_lvar : (('a,'b,'c,'d) #Env.O.t, 'd) Env.value -> string list -> unit

val check_value2 :
  ('a -> int array -> 'a) ->
  ('b -> int array -> 'b) ->
  (('c,'d,'e,'f) #Env.O.t as 'g, 'a) Env.value -> ('g, 'b) Env.value ->
  'g * 'a * 'b
val mapbinop :
  ('a -> int array -> 'a) ->
  ('b -> int array -> 'b) ->
  ('a -> 'b -> 'c) ->
  (('d,'e,'f,'g) #Env.O.t as 'h, 'a) Env.value -> ('h, 'b) Env.value ->
  ('h, 'c) Env.value
val mapbinope :
  ('a -> int array -> 'a) ->
  ('b -> int array -> 'b) ->
  ('d -> 'a -> 'b -> 'c) ->
  (('e,'f,'g,'h) #Env.O.t as 'd, 'a) Env.value -> ('d, 'b) Env.value ->
  ('d, 'c) Env.value
val check_value3 :
  ('a -> int array -> 'a) ->
  ('b -> int array -> 'b) ->
  ('c -> int array -> 'c) ->
  (('d,'e,'f,'g) #Env.O.t as 'h, 'a) Env.value ->
...
val mapterop :
  ...
  ('c -> int array -> 'c) ->
  ('a -> 'b -> 'c -> 'd) ->
  (('e,'f,'g,'h) #Env.O.t as 'i, 'a) Env.value ->
...
val check_lvarvalue :
  ('a -> int array -> 'a) ->
  ('b -> int array -> 'b) ->
  (('c,'d,'e,'f) #Env.O.t as 'g, 'a) Env.value ->
